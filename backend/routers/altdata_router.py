"""Alt-Data Intelligence endpoint — GET /api/v6/altdata/{ticker}."""
...
from quantedge.scoring.altdata_features import compute_altdata_features
from quantedge.scoring.compute import score_signal
from quantedge.scoring.cat_altdata_v6 import CATEGORIES, altdata_rating
...
def score_altdata(features):
    ...
    s,c=_roll(cats)
    return {"label":"Alt-Data Intelligence","weight":3.0,"score":s,"confidence":c,"categories":cats}
...
async def _news(ticker, api_key):
    try:
        async with httpx.AsyncClient(timeout=12) as c:
            r=await c.get(f"https://api.polygon.io/v2/reference/news?ticker={ticker}&limit=50&apiKey={api_key}")
            if r.status_code==200: return (r.json() or {}).get("results",[])
    except Exception: pass
    return []

async def compute_altdata_intelligence(ticker: str, api_key: str) -> Dict[str,Any]:
    ticker=ticker.upper().strip()
    bars=await _bars(ticker, api_key)
    if not bars or len(bars)<30:
        return {"ticker":ticker,"available":False,"reason":"insufficient price history"}
    news=await _news(ticker, api_key)
    global _CIK
    if not _CIK:
    ...
    feats=compute_altdata_features(bars, news, insider=insider)
    if feats.get("available") is False:
        return {"ticker":ticker,"available":False,"reason":"could not compute alt-data signals"}
    tree=score_altdata(feats)
    n_scored=sum(c["n_scored"] for c in tree["categories"]); n_total=sum(c["n_signals"] for c in tree["categories"])
    return {"ticker":ticker,"available":True,"intelligence":"altdata",
            "score":tree["score"],"confidence":tree["confidence"],
            "altdata_rating":altdata_rating(tree["score"]),"weight_in_conviction":3.0,
            "coverage":{"scored":n_scored,"total":n_total},"tree":tree,
            "key_metrics":{k:feats.get(k) for k in
                ["accumulation_ratio","obv_slope","unusual_volume","news_velocity","news_sentiment_mean",
                 "txn_count_surge","dollar_volume_trend","amihud_illiquidity","volume_price_divergence","avg_trade_size_trend"]}}

@router.get("/altdata/{ticker}")
async def get_altdata(ticker: str, http_request: Request,
    ...
    return {"data":_san(await compute_altdata_intelligence(ticker, api_key))}
