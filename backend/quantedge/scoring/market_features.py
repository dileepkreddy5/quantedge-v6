...
DEEP_CATEGORIES = {
 "volatility": ("Volatility Intelligence", 0.60, [
   ("vol_percentile","Volatility percentile","vol_percentile",0.40,80,40,False,"current vol vs own history (lower=calmer)"),
   ("vol_trend","Volatility trend","vol_trend",0.30,0.2,-0.1,False,"rising vol is a risk"),
   ("beta","Beta","beta",0.30,1.4,0.9,False,"market sensitivity (lower=defensive)"),
 ]),
 "trading_risk": ("Trading Risk", 0.55, [
   ("max_drawdown","Max drawdown","max_drawdown",0.30,-0.35,-0.15,True,"worst peak-to-trough (closer to 0 better)"),
   ("var_5pct","5% Value-at-Risk","var_5pct",0.30,-0.04,-0.02,True,"daily downside tail"),
   ("gap_risk","Gap risk frequency","gap_risk_freq",0.20,0.20,0.08,False,"frequency of >2% daily moves"),
   ("downside_dev","Downside deviation","downside_dev",0.20,0.30,0.15,False,"annualized downside volatility"),
 ]),
 "volume_accum": ("Volume & Accumulation", 0.30, [
   ("obv_slope","OBV accumulation","obv_slope",0.40,-0.5,1.0,True,"on-balance-volume trend"),
   ("vwap_distance","VWAP distance","vwap_distance",0.30,-0.05,0.03,True,"price vs volume-weighted avg"),
   ("rvol","Relative volume","rvol",0.30,0.5,1.5,True,"volume vs 21-day average"),
 ]),
